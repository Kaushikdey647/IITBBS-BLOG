import os
import secrets
from flask import url_for, current_app
from flask_mail import Message
from flaskblog import mail
from flask_login import current_user

def save_picture(form_picture):
    random_hex = secrets.token_hex(8)
    _,f_ext = os.path.splitext(form_picture.filename)
    picture_fn = random_hex + f_ext 
    picture_path = os.path.join(current_app.root_path,"static/profile_pics",picture_fn)
    form_picture.save(picture_path) 
    prev_picture = os.path.join(current_app.root_path, 'static/profile_pics', current_user.image_file)
    ok = os.path.exists(prev_picture)
    if os.path.exists(prev_picture) and current_user.image_file != "default.jpg":
        os.remove(prev_picture)
    return picture_fn

def send_reset_email(user):
    token = user.get_reset_token()
    email_user = os.environ.get('EMAIL_USER')
    msg = Message("Password Reset Request",sender="email_user",recipients=[user.email])
    msg.body = f"""To reset your password, visit the following link:
{url_for("users.reset_token",token=token,_external=True)}

If you did not make this request then simply ignore this email and no changes will be made.
"""
    mail.send(msg)

def send_confirm_email(user):
    token = user.get_reset_token()
    email_user = os.environ.get('EMAIL_USER')
    msg = Message("Confirm Email",sender="email_user",recipients=[user.email])
    msg.body = f"""To confirm your email, visit the following link:
{url_for("users.confirm_token",token=token,_external=True)}

If you did not make this request then simply ignore this email and no changes will be made.
"""
    mail.send(msg)